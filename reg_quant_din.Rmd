...
Considere a função perda $\rho_\tau (u) = u(\tau - \mathbb{I}(u < 0))$. `r Citet(myBib, "koenker:2005")` mostrou que o valor que minimiza a perda esperada $E_F(\rho_\tau (Y - q)$ satisfaz $F(q) = \tau$, ou ainda $q = Q_\tau(Y)$.
...
**Propriedade:** `r Citep(myBib, "kotz:2001")` é possível mostrar que, se $Y|\mu,\sigma \sim LA(\mu, \sigma, \tau)$, então podemos escrever hierarquicamente:
